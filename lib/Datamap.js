...
module.exports = class Datamap {
   ...
   *
   * @const { Datamap.Options } Datamap.Options
   */
  static get Options () {
    return Options
  }
  ...
  * iterator () {
  ...
  add (value) {
    if (!this.$options.index) {
      throw new InvalidArgument(`No index set for ${this.constructor.name}`)
    }

    const key = value[this.$options.index]

    if (key == null) {
      throw new InvalidArgument('No index value found')
    }
   ...
   *
   * @method Datamap#setIndex
   * @param { string } index          Any value
   */
  setIndex (value) {
