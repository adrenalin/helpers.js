...
module.exports = class Localization {
  ...
  getInLang (lang, locale, ...args) {
    ...
    return String(localized)
      ...
      .replace(/(\\)?%([dmnps])([0-9]*)(\[(.+)\])?/g, (r, escaped, type, index, paramsWrapped, params) => {
        if (escaped) {
          return r.replace(/^\\/, '')
        }
        ...
        switch (type) {
          case 'd':
            return this.dateFormat(parameters[index], { lang, format: params })

          case 'm':
            return this.moneyFormat(parameters[index], { lang, precision: params == null ? null : parseInt(params) })

          case 'n':
            return this.numberFormat(parameters[index], { lang, precision: params == null ? null : parseInt(params) })
  ...
  numberFormat (value, ...options) {
    ...
    if (isObject(options[0])) {
      if (options.length > 1) {
        throw new Error('Either give a configuration object or spread by precision, lang, decimal and thousand')
      }
