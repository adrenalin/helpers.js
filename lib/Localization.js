...
module.exports = class Localization {
  ...
  quantityFormat (value, options) {
    ...
    const v = this.constructor.getRoundedNumber(value, opts.precision, opts.rounding)

    const formatValue = (k = '') => {
      k = k || ''
      return `${this.numberFormat(this.constructor.getRoundedNumber(value / q[k], opts.precision, opts.rounding), { precision: opts.precision })} ${k}${opts.unit}`.replace(/ +$/, '')
    }

    let prev = Object.keys(q)[0]
