 ...
 */
class InvalidArgument extends LocalizationError {}
...
module.exports = class Localization {
  ...
  numberFormat (value, ...options) {
    ...
    if (isObject(options[0])) {
      if (options.length > 1) {
        throw new Error('Either give a configuration object or spread by precision, lang, decimal, thousand, rounding')
      }
    ...
    if (opts.precision != null) {
      opts.rounding = opts.rounding || 'round'
    }
    ...
    // Rounding
    if (opts.precision != null && opts.rounding) {
      if (!['round', 'ceil', 'floor'].includes(opts.rounding)) {
        throw new InvalidArgument(`Only 'round', 'ceil', 'floor' are accepted as rounding methods, now got '${opts.rounding}'`)
      }

      const pow = Math.pow(10, opts.precision)
      value = Math[opts.rounding](value * pow) / pow
    }
