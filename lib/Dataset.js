...
module.exports = class Dataset extends Set {
  ...
  constructor (data, options) {
    ...
    if ($.options.id) {
      $.indices[$.options.id] = $.mapped = new Map()
    }
  ...
  /**
   * Alias addToIndex for addToIndices
   *
   * @property { function } Dataset#ddToIndex
   */
  get addToIndex () {
    return this.addToIndices
  }
  ...
  getByIndex (index, id) {
    try {
      return this.$.indices[index].get(id)
    } catch (err) {
      return undefined
    }
  }
   ...
   * @return { Dataset }              Dataset of the values returned by the iterator
   */
  map (iterator) {
    const d = new Dataset(null, this.$.options)
    this.forEach(function map (item) {
      return d.add(iterator(item))
    })
    return d
  }
   ...
   *
   * @method Dataset#splitIntoChunks
   * @param { number } length         Number of chunks
   * @returns { Dataset[] }           An array of datasets
   */
  splitIntoChunks (length) {
    const l = typeof length === 'number' ? length : Number(length)

    if (isNaN(l) || Math.round(l) !== l || l < 1) {
      throw new InvalidArgument('Dataset.splitIntoChunks expects the length to be a positive integer')
    }

    const s = Math.ceil(this.size / length)
    const rval = []
    const values = this.toArray()

    for (let i = 0; i < l; i++) {
      const subset = values.splice(0, s)

      if (!subset.length) {
        break
      }

      rval.push(new Dataset(subset, this.$.options))
    }
